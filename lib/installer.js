...
function Installer(config) {
    ...
    this.silent = config.silent || sh.config.silent;
...
Installer.prototype._update = function (mod) {
    ...
    sh.echo('   Updating the `' + mod + '` repo from remote');
...
Installer.prototype._install = function (mod) {
    var local = path.join(this.root, mod);
    var url = path.join(this.gitbase, mod + '.git');
    sh.mkdir(local);

    sh.echo('   Fetching the `' + mod + '` repo from remote');
